...
JOB_COUNTER_FILE="/tmp/clashnivo_jobs"
...
unify_ps_prevent() {
	if [ "$(ps --version 2>&1 |grep -c procps-ng)" -eq 1 ]; then
		echo "$(ps -efw |grep -v grep |grep -c "/etc/init.d/clashnivo")"
	else
		echo "$(ps -w |grep -v grep |grep -c "/etc/init.d/clashnivo")"
	fi
}

unify_ps_cfgname() {
	if [ "$(ps --version 2>&1 |grep -c procps-ng)" -eq 1 ]; then
		echo "$(ps -efw |grep /etc/clashnivo/core/mihomo 2>/dev/null |grep -v grep |awk -F '-f ' '{print $2}' 2>/dev/null)"
	else
		echo "$(ps -w |grep /etc/clashnivo/core/mihomo 2>/dev/null |grep -v grep |awk -F '-f ' '{print $2}' 2>/dev/null)"
	fi
}

inc_job_counter() {
   exec 999>"/tmp/lock/clashnivo_jobs.lock"
   flock -x 999
   local cnt=0
   local restart=0
...
dec_job_counter_and_restart() {
   ...
   if [ $cnt -eq 0 ] && [ "$restart" -eq 1 ] && [ "$(unify_ps_prevent)" -eq 0 ]; then
      /etc/init.d/clashnivo restart >/dev/null 2>&1 &
