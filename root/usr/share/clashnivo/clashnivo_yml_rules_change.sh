...
# smart group auto-switch, router self-proxy rule injection, and the
# positional-arg tail for smart/LightGBM are dropped.
. /lib/functions.sh
...
#   $1  enable_custom_clash_rules   ($4-$7 are accepted but unused —
#   $2  CONFIG_FILE path             the router-self-proxy rule injection
#   $3  BT/PT DIRECT toggle          they fed has been removed.)
...
yml_other_set()
{
   ...
   begin
      ...
      thread_pool << Thread.new{
         ...
         begin
            if $3 == 1 then
               if system('strings /etc/clashnivo/GeoSite.dat /etc/clashnivo/GeoSite.dat |grep -i category-public-tracker >/dev/null 2>&1') then
         ...
         begin
            CUSTOM_RULE = File::exist?('/etc/clashnivo/custom/clashnivo_custom_rules.list') ? YAML.load_file('/etc/clashnivo/custom/clashnivo_custom_rules.list') : {};
            CUSTOM_RULE_2 = File::exist?('/etc/clashnivo/custom/clashnivo_custom_rules_2.list') ? YAML.load_file('/etc/clashnivo/custom/clashnivo_custom_rules_2.list') : {};
         ...
         begin
            if $1 == 1 then
               custom_files = [
                  { file: '/etc/clashnivo/custom/clashnivo_custom_rules.list', position: 'top' },
                  { file: '/etc/clashnivo/custom/clashnivo_custom_rules_2.list', position: 'bottom' }
               ];
                  ...
                  if File::exist?(file_info[:file]) then
                     ...
                     valid_rules = transformed_rules.select{|x|
                        ...
                        if CONFIG_GROUP.include?(RULE_GROUP) then
                           ...
                           YAML.LOG_WARN('Skipped The Custom Rule Because Group & Proxy Not Found: [' + x + ']');
                           false;
                        end;
                     };
