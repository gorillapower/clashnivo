...
# LightGBM / chnroute / IPv6 / geoasn / custom_fallback_filter / dashboard
# selector dropped. 32 positional args — see arg contract below.
. /usr/share/clashnivo/ruby.sh
. /usr/share/clashnivo/log.sh
. /usr/share/clashnivo/uci.sh
. /lib/functions.sh

# Positional argument contract (must match init.d start_service call site):
#   $1  fake-ip-mode (fake-ip|redir-host)   ${17} append-sniffer-config
#   $2  dashboard secret                    ${18} interface-name
#   $3  external-controller port            ${19} tcp-concurrent
#   $4  redir-port                          ${20} add-default-from-dns
#   $5  CONFIG_FILE path                    ${21} sniffer-parse-pure-ip
#   $6  http-port                           ${22} find-process-mode
#   $7  socks-port                          ${23} fake-ip-range
#   $8  log-level                           ${24} unified-delay
#   $9  proxy-mode (rule|global|direct)     ${25} respect-rules
#  ${10} dns-listen-port                    ${26} fake-ip-filter-mode
#  ${11} mixed-port                         ${27} routing-mark-setting
#  ${12} tproxy-port                        ${28} quic-go-disable-gso
#  ${13} store-fake-ip                      ${29} cors-origin
#  ${14} enable-sniffer                     ${30} geo-custom-url (MMDB)
#  ${15} geodata-mode (enable_geoip_dat)    ${31} geoip-custom-url
#  ${16} geodata-loader                     ${32} geosite-custom-url
...
dashboard_type=$(uci_get_config "dashboard_type" || echo "zashboard")
fake_ip_filter_mode=${26}

if [ "$1" = "fake-ip" ] && [ "$enable_redirect_dns" != "2" ]; then
   TMP_FILTER_FILE="/tmp/yaml_clashnivo_fake_filter_include"
   > "$TMP_FILTER_FILE"
fi
...
# Dashboard auth list (UCI `authentication` section).
yml_auth_get()
{
   local section="$1"
   ...
   if [ -z "$username" ] || [ -z "$password" ]; then
      ...
      LOG_TIP "You have set the authentication of SOCKS5/HTTP(S) proxy with [$username:$password]..."
      echo "  - $username:$password" >>/tmp/yaml_clashnivo_auth
   fi
}

# Custom DNS dispatcher (UCI dns_servers sections + WAN DNS append).
yml_dns_custom()
{
   if [ "$1" = 1 ] || [ "$3" = 1 ]; then
      sys_dns_append "$3"
...
# IPv4-only WAN DNS append: DHCP / PPPoE / gateway.
sys_dns_append()
{
   if [ "$1" = 1 ]; then
...
PROXY_GROUPS=$(ruby -ryaml -rYAML -I "/usr/share/clashnivo" -E UTF-8 -e "
   begin
...
yml_dns_get()
{
   ...
   local enabled port type ip group dns_type dns_address interface specific_group node_resolve direct_nameserver http3 skip_cert_verify ecs_subnet ecs_override disable_qtype_param disable_qtype disable_ipv4 disable_reuse
...
yml_dns_custom "$enable_custom_dns" "$5" "$append_wan_dns"
...
begin
   ...
   geodata_mode = '${15}' == '1'
   geodata_loader = '${16}'
   append_sniffer_config = '${17}' == '1'
   interface_name = '${18}'
   tcp_concurrent = '${19}' == '1'
   add_default_from_dns = '${20}' == '1'
   sniffer_parse_pure_ip = '${21}' == '1'
   find_process_mode = '${22}'
   fake_ip_range = '${23}'
   unified_delay = '${24}' == '1'
   respect_rules = '${25}' == '1'
   fake_ip_filter_mode = '${26}'
   routing_mark_setting = '${27}'
   quic_gso = '${28}' == '1'
   cors_origin = '${29}'
   geo_custom_url = '${30}'
   geoip_custom_url = '${31}'
   geosite_custom_url = '${32}'
   ...
   threads << Thread.new do
      begin
         ...
         Value['secret'] = secret
         Value['bind-address'] = '*'
         Value['external-ui'] = '/usr/share/clashnivo/ui'
         case dashboard_type
         when 'metacubexd'
         ...
         if enable_sniffer
            ...
            if append_sniffer_config && (custom_sniffer = safe_load_yaml('/etc/clashnivo/custom/clashnivo_custom_sniffer.yaml'))
      ...
      begin
         if enable_custom_dns || append_wan_dns
            if (namedns_config = safe_load_yaml('/tmp/yaml_config.namedns.yaml')) && namedns_config['nameserver']
               ...
               YAML.LOG_ERROR('Nameserver Option Must Be Set, Stop Customising DNS Servers')
            end
         end
      rescue Exception => e
   ...
   threads << Thread.new do
      begin
         if custom_name_policy
            if (policy = safe_load_yaml('/etc/clashnivo/custom/clashnivo_custom_domain_dns_policy.list'))
   ...
   threads << Thread.new do
      begin
         if custom_proxy_server_policy
            if (policy = safe_load_yaml('/etc/clashnivo/custom/clashnivo_custom_proxy_server_dns_policy.list'))
   ...
   # Fake-IP Filter (user-driven only — no chnroute auto-injection)
   threads << Thread.new do
      begin
         if custom_fakeip_filter
            Value['dns']['fake-ip-filter-mode'] = fake_ip_filter_mode
            if fake_ip_mode == 'fake-ip'
               merge_list_from_file(Value['dns'], 'fake-ip-filter', '/etc/clashnivo/custom/clashnivo_custom_fake_filter.list')
               merge_list_from_file(Value['dns'], 'fake-ip-filter', '/tmp/yaml_clashnivo_fake_filter_include')
            end
         end
      rescue Exception => e
   ...
   threads << Thread.new do
      begin
         if custom_host
            if (hosts_content = safe_load_yaml('/etc/clashnivo/custom/clashnivo_custom_hosts.list')) && !hosts_content.empty?
   ...
   threads << Thread.new do
      begin
         if (auth_config = safe_load_yaml('/tmp/yaml_clashnivo_auth'))
   ...
   begin
                  ...
                  next unless Value['dns'].key?(opt) && !Value['dns'][opt].nil?
                     if opt != 'nameserver-policy' && opt != 'proxy-server-nameserver-policy'
                        ...
                        if Value['dns'][opt].size < original_size
                           YAML.LOG_TIP('Option [%s] is Set to [system] as DNS Server Which May Cause DNS Loop, Already Removed...' % [opt])
                        end
                     else
                        Value['dns'][opt].each do |k, v|
                           if v.is_a?(Array)
                              ...
                              if v.empty?
                                 ...
                                 YAML.LOG_TIP('Option [%s - %s] is Set to [system] as DNS Server Which May Cause DNS Loop, Already Removed...' % [opt, k])
                              elsif v.size < original_size
                                 YAML.LOG_TIP('Option [%s - %s] is Set to [system] as DNS Server Which May Cause DNS Loop, Already Removed...' % [opt, k])
                              end
                           elsif v.to_s.match?(/^system($|:\/\/)/)
                              Value['dns'][opt].delete(k)
                              YAML.LOG_TIP('Option [%s - %s] is Set to [%s] as DNS Server Which May Cause DNS Loop, Already Removed...' % [opt, k, v.to_s])
                           end
                        end
                     end
               rescue Exception => e
      ...
      if Value.dig('dns', 'proxy-server-nameserver').to_a.empty?
         ...
         if respect_rules || Value.dig('dns', 'respect-rules').to_s == 'true' || all_ns_proxied || proxy_server_nameserver_policy
            ...
            if all_ns_proxied
               YAML.LOG_TIP('Nameserver Option All Set to Proxy, Auto Set Proxy-server-nameserver to [%s] To Avoid Proxy Resolve Loop...' % [default_proxy_servers.join(', ')])
            elsif proxy_server_nameserver_policy
               YAML.LOG_TIP('[Proxy-server-nameserver-policy] Requires Proxy-server-nameserver To Be Set, Auto Set to [%s]' % [default_proxy_servers.join(', ')])
            else
               YAML.LOG_TIP('[Respect-rules] Requires Proxy-server-nameserver To Be Set, Auto Set to [%s]' % [default_proxy_servers.join(', ')])
            end
         end
      else
         ...
         if all_psn_proxied
            ...
            YAML.LOG_TIP('Proxy-server-nameserver Option All Set to Proxy, Auto Set to [%s] To Avoid Proxy Resolve Loop...' % [default_proxy_servers.join(', ')])
         end
      end
   rescue Exception => e
      YAML.LOG_ERROR('Config File params check Failed, [%s]' % [e.message])
   end
rescue Exception => e
