...
write_ruby_part()
{
  local part="$1" sid="${CLASHNIVO_OVERWRITE_SID:-unknown}"
  if [ -z "$part" ]; then
    return
  fi
  if [ "$OVERWRITE_PARENT" = "yaml_overwrite" ]; then
    mkdir -p /tmp/yaml_clashnivo_ruby_parts 2>/dev/null
    echo "threads << Thread.new do begin $part; rescue Exception => e; YAML.LOG_ERROR('Set Custom Overwrite Script Failed,【%s】' % [e.message]); end; end" >> "/tmp/yaml_clashnivo_ruby_parts/$sid"
  else
    echo "threads << Thread.new do begin $part; rescue Exception => e; YAML.LOG_ERROR('Set Custom Overwrite Script Failed,【%s】' % [e.message]); end; end" >> /tmp/yaml_clashnivo_ruby_parse
  fi
}
...
run_ruby_part()
{
  ...
  if [ "$show_error" = "false" ] || [ "$show_error" = "0" ]; then
    ruby -ryaml -rYAML -I "/usr/share/clashnivo" -E UTF-8 -e "begin $part; end" 2>/dev/null
  else
    ruby -ryaml -rYAML -I "/usr/share/clashnivo" -E UTF-8 -e "begin $part; rescue Exception => e; YAML.LOG_ERROR('Set Custom Overwrite Script Failed,【%s】' % [e.message]); end" 2>/dev/null
  fi
}

clashnivo_custom_overwrite() {
  local pid=$$
  while [ "$pid" != "1" ]; do
    pname=$(tr -d '\0' < /proc/"$pid"/cmdline 2>/dev/null)
    case "$pname" in
      *clashnivo_custom_overwrite.sh*)
...
if clashnivo_custom_overwrite; then
